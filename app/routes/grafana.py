...
def grafana_workouts(request: Request):
    ...
    start_date = request.query_params.get("from", "") or request.query_params.get(
        "start_date", ""
    )
    end_date = request.query_params.get("to", "") or request.query_params.get(
        "end_date", ""
    )
    try:
        ...
        return JSONResponse(
            {
                "error": "Unsupported field selection.",
                "allowed_fields": config.GRAPHABLE_FIELDS,
            },
            status_code=400,
        )

    filtered_data = filter_data(
        load_history_file(config.HISTORY_FILE), start_date, end_date
    )
    points: list[dict] = []
    for _, row in filtered_data.iterrows():
        ...
        for field in selected_fields:
            ...
            if not pd.isna(value):
                points.append(
                    {"time": timestamp, "field": field, "value": float(value)}
                )
    return points
...
def grafana_summary(request: Request):
    ...
    for field in selected_fields:
        ...
        summary["averages"][field] = (
            None if field_series.empty else float(field_series.mean())
        )
        summary["minimums"][field] = (
            None if field_series.empty else float(field_series.min())
        )
        summary["maximums"][field] = (
            None if field_series.empty else float(field_series.max())
        )
    return summary
